  ...
  <section class="demo-and-code-wrapper">
    ...
    <div ref="codeControl"
      ...
      <div class="online-wrapper"
        @click.stop>
        <OnlineEdit v-for="platform in platforms"
          :key="platform"
...
export default {
  ...
  computed: {
    ...
    // animation
    codeWrapperStyle: vm => ({
      height: vm.isShowCode ? `${vm.codeHeight}px` : `${vm.minHeight}px`
    }),
    // sticky
    codeControlStyle: vm => ({
    ...
    parsedCode: vm => {
      ...
      const html =
        getMatchedResult(HTML_RE)(source) ||
        source
          .replace(JS_RE, "")
          .replace(CSS_RE, "")
          .replace(HTML_RE, "")
  ...
  methods: {
    ...
    getDomRect() {
      ...
      const {
        top: codeTop,
        height: codeHeight
      } = codeWrapper.firstChild.getBoundingClientRect();
